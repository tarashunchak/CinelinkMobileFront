...
import { BackdropBlur, Canvas, ColorMatrix, Fill, FractalNoise, LinearGradient, LumaColorFilter, Mask, RadialGradient, rect, Rect, RoundedRect, rrect, vec } from "@shopify/react-native-skia";
...
export default function BottomBar({ blurTarget }: { blurTarget: any }) {
  const height = 52;
  const width = wp(92);
  const r = rrect(rect(0, 0, width, height), 22, 22);
  return (
    <View style={{ width, height, position: "absolute", bottom: wp(4), alignSelf: "center", overflow: "hidden", borderRadius: 22 }}>
      <BlurView
        tint="dark"
        intensity={40}
      ...
      <Canvas style={StyleSheet.absoluteFill}>
        ...
        <RoundedRect rect={r} style="stroke" strokeWidth={2} >
          <RadialGradient
            c={vec(width / 2, 0)}
            r={85}
            colors={["white", "rgba(120, 120, 120, 0.1)"]}
          />
        </RoundedRect>
        <RoundedRect rect={rrect(rect(1, 1, width - 2, height - 2), 22, 22)} style="stroke" strokeWidth={0.5} >
          <LinearGradient
            start={vec(0, 0)}
